      ...
      .then(function(filesize) {
        ...
        if (range) {
    		...
    		console.log(filesize);
          	...
          	transcodeChunk(filepath, null, res);
        }
...
var webmOptions = [
  ...
  '-b:v', '500k', //bitrate for the video
...
var transcodeChunk = function(file, range, res) {
	if (!started[file]) {
		console.log("here");
	  started[file] = true;
	  ffmpeg = spawn('ffmpeg', webmOptions);
	  fs.createReadStream(file).pipe(ffmpeg.stdin);
	  ffmpeg.stdout.pipe(fs.createWriteStream("output.webm"));
	  setTimeout(function(){
	  	pump(fs.createReadStream("output.webm"), res);
	  },10000)
	} else {
		console.log("here1");
		pump(fs.createReadStream("output.webm", range), res);
