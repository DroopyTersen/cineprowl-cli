...
var streamMovie = function(req, res, filepath) {
	...
	fs.stat(filepath, function(err, stats){
    	...
    	res.setHeader('Content-Type', mime.lookup("webm"));
    	...
    	if (!range) {
    		...
    		var fullStream = ffmpeg(fs.createReadStream(filepath))
    			//.preset("flashvideo") 
    			.preset(mobilePreset)
			    .outputOptions(["-ftp-write-seekable 1"])
        	...
        	var rangeStream = ffmpeg(fs.createReadStream(filepath, range))
    			//.preset("flashvideo") 
    			.preset(mobilePreset)
			    .outputOptions(["-ftp-write-seekable 1"])
        		.on('error', function(err){
